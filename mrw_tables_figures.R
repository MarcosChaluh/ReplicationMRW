...
mrw <- read_csv("data/mrw.csv") %>%
  select(-number) %>%
  mutate(
    lngd = log(popgrowth / 100 + 0.05),   #  g + delta is 5%
    ls = log(i_y / 100),
    ls_lngd = ls - lngd,
    lschool = log(school),
...
write_csv(table_unres, file.path(tables_dir, "mrw_table_unrestricted.csv"))

table_res <- build_table(models_res, term_labels_restricted, stat_alpha_res, digits = 2)
write_csv(table_res, file.path(tables_dir, "mrw_table_restricted.csv"))
...
write_csv(table_hc_unres, file.path(tables_dir, "mrw_table_human_capital_unrestricted.csv"))

table_hc_res <- build_table(models_hc_res, term_labels_hc_res_table, stat_alpha_beta_res, digits = 2)
write_csv(table_hc_res, file.path(tables_dir, "mrw_table_human_capital_restricted.csv"))
...
write_csv(table_roc, file.path(tables_dir, "mrw_table_unconditional_convergence.csv"))
...
write_csv(table_roc_cond, file.path(tables_dir, "mrw_table_conditional_convergence.csv"))
...
write_csv(table_roc_cond_hc, file.path(tables_dir, "mrw_table_conditional_convergence_human_capital.csv"))
...
write_csv(table_roc_res, file.path(tables_dir, "mrw_table_conditional_convergence_restricted.csv"))
...
ggsave(file.path(figures_dir, "mrw_conditional_convergence.png"), convergence_plot,
       width = 8, height = 10, dpi = 300)

print("Tables written to results/tables")
print("Figure written to results/figures/mrw_conditional_convergence.png")